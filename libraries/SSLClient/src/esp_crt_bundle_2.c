...
typedef struct crt_bundle_t_2 {
    const uint8_t **crts;
    uint16_t num_certs;
    size_t x509_crt_bundle_len;
} crt_bundle_t_2;

static crt_bundle_t_2 s_crt_bundle;

static int esp_crt_verify_callback_2(void *buf, mbedtls_x509_crt *crt, int data, uint32_t *flags);
static int esp_crt_check_signature_2(mbedtls_x509_crt *child, const uint8_t *pub_key_buf, size_t pub_key_len);
...
*/
int esp_crt_verify_callback_2(void *buf, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
    ...
    if (crt_found) {
        ...
        ret = esp_crt_check_signature_2(child, s_crt_bundle.crts[middle] + CRT_HEADER_OFFSET + name_len, key_len);
    }
 ...
 */
static esp_err_t esp_crt_bundle_init_2(const uint8_t *x509_bundle)
{
...
esp_err_t esp_crt_bundle_attach_2(void *conf)
{
    ...
    if (conf) {
        ...
        mbedtls_ssl_conf_verify(ssl_conf, esp_crt_verify_callback_2, NULL);
    }
...
void esp_crt_bundle_set_2(const uint8_t *x509_bundle)
{
    ...
    free(s_crt_bundle.crts);
    esp_crt_bundle_init_2(x509_bundle);
}
